...
def generate_food(snake_list):
    ...
    while True:
        ...
        if [foodx, foody] not in snake_list:
            return foodx, foody, random.choice([1, 2, 3]) #Рандомный вес
...
foodx, foody, food_weight = generate_food(snake_list)
...
while not game_over:
    ...
    if pygame.time.get_ticks() - last_food_time > timer_duration:
        foodx, foody, food_weight = generate_food(snake_list)
        last_food_time = pygame.time.get_ticks()
    #Отрисовка еды
    ...
    if x == foodx and y == foody:
        snake_length += food_weight  #Увеличение длины змеи на вес еды
        score += food_weight  #Добавление очков
        foodx, foody, food_weight = generate_food(snake_list)
        last_food_time = pygame.time.get_ticks()
